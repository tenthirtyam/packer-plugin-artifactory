...
upload the artifact to Artifactory using basic (username / password) authentication with
custom `properties`, also applied to the `artifact_path` folder via `artifact_path_properties`.

Same defaults and platforms as [`basic/`](../basic/).
...
cd examples/basic-with-properties
packer init .
# OVA (default)
